(function(){
  ...
  function installHelp(){
    ...
    document.querySelectorAll('input:not([type=hidden]):not([type=button]):not([type=submit]),select,textarea').forEach(el=>{
      ...
      const icon=document.createElement('span');icon.className='help-icon';icon.textContent='i';icon.tabIndex=0;icon.setAttribute('aria-label','Information');label.appendChild(icon);
      const show=e=>{tip.textContent=helpText(el,label);tip.style.display='block';move(e)};
      ...
      icon.addEventListener('focus',()=>{tip.textContent=helpText(el,label);tip.style.display='block';const r=icon.getBoundingClientRect();tip.style.left=Math.min(r.right+8,innerWidth-tip.offsetWidth-12)+'px';tip.style.top=r.top+'px'});icon.addEventListener('blur',()=>tip.style.display='none');
    });
  }
